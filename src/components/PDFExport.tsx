...
function generatePDFContent(
  ...
  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>WhereToFIRE Report</title>
<style>
...
<body>
<div class="header"><h1>🔥 WhereToFIRE Report</h1><p>Comparing early retirement in ${country1?.name} vs ${country2?.name}</p><p style="font-size:12px;margin-top:5px">Generated ${new Date().toLocaleDateString()}</p></div>
...
<div class="footer"><p>Generated by WhereToFIRE • wheretofire.com</p><p>This is an estimate only. Consult with tax and immigration professionals.</p></div>
</body></html>`;
}
...
export function PDFExportButton({ result1, result2, comparison, country1Code, country2Code, inputs }: PDFExportProps) {
  ...
  return (
    <button onClick={generatePDF} disabled={isGenerating} className="hidden sm:flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors disabled:opacity-50">
      {isGenerating ? (
